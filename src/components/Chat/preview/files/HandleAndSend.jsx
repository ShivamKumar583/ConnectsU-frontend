...
function HandleAndSend({ activeIndex, setActiveIndex, message, socket }) {
  ...
  return (
    <div className="w-[97%] flex items-center justify-between mt-2 border-t dark:border-slate-600">
      {/*Empty*/}
      <span></span>
      {/*List files*/}
      <div className="flex items-center gap-x-2">
        {files.map((file, i) => (
          <div
            key={i}
            className={`fileThumbnail relative w-14 h-14 border dark:border-slate-200 mt-2 rounded-md overflow-hidden cursor-pointer
            ${activeIndex === i ? "border-[3px] !border-purple-600" : ""}
            `}
      ...
      {/*Send button*/}
      <div
        className="bg-purple-600 w-16 h-16 mt-2 rounded-full flex items-center justify-center cursor-pointer"
        onClick={(e) => sendMessageHandler(e)}
      >
        {loading ? (
          <ClipLoader color="#E9EDEF" size={25} />
        ) : (
          <SendIcon className="fill-slate-300" />
        )}
      </div>
    </div>
  );
